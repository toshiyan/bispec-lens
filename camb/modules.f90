...
500     if (L< lmax_extrap_highl) &
    ...
    if (CP%WantScalars .and. ScalFile /= '') then
        ...
        do in=1,CP%InitPower%nn
            do il=lmin,min(10000,CP%Max_l)
                write(fileio_unit,trim(numcat('(1I6,',last_C))//'E20.12)')il ,fact*Cl_scalar(il,in,C_Temp:last_C)
            end do
            do il=10100,CP%Max_l, 100
                write(fileio_unit,trim(numcat('(1E20.12,',last_C))//'E24.16)') real(il),&   !namikawa
    ...
    fmt = '('//trim(adjustl(fmt))//'E24.16)' !namikawa
