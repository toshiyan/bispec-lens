...
fname = root + 'modelw/Pk/Pklin_new.dat'
...
#for zn, zsi in [(25,2.5),(30,3.),(30,4.),(100,zcmb)]:
    ...
    for i, Lmax in enumerate(Lmaxs):
        ...
        if kk_noise == 'advact':
            nlkk[2:] = np.loadtxt(root+'nldd/advact_s6_t1_rlmax4000.dat',unpack=True)[1][:Lmax-1]
        if kk_noise == 'so':
            nlkk[2:] = np.loadtxt(root+'nldd/so/kappa_deproj0_sens2_16000_lT30-3000_lP30-5000.dat',unpack=True)[7][:Lmax-1]
        if kk_noise == 's4':
            nlkk[2:] = np.loadtxt(root+'nldd/S4_s1_t3_rlmax4000.dat',unpack=True)[1][:Lmax-1]
